...
namespace FlatManagement.Test.Common
{
	public class FlatShould
	{
		...
		{
			Flat flat = new Flat() { Name = name, Address = address };
			flat.Validate();
			Assert.True(flat.ValidationResult.IsValid);
		}
		...
		public void BeInvalid(string name, string address)
		{
			Flat flat = new Flat() { Name = name, Address = address };
			flat.Validate();
			Assert.False(flat.ValidationResult.IsValid);
			Assert.NotEmpty(flat.ValidationResult.Messages);
		}
	}
}
